...
def login(payload: user.login, response: Response):
    ...
    try:
        ...
        return {
            "success": False,
            "detail": "Email address not found. Please check your email or sign up for a new account."
        }
...
def create(payload: user.create, response: Response):
    ...
    userCreated = User.createUser(
        email=payload.email, password=password, userName=payload.userName)
    if userCreated == True:
...
def changeUserName(userId: int, payload: user.changeUserName, response: Response, token: str = Header()):
    ...
    if tokenVerification['valid'] == True:
        ...
        if tokenVerification['payload']['user-id'] == requestedUser:
            changedUserName = User.changeUserName(
                userName=payload.newUserName, id=requestedUser)
...
def getUserLeagues(userId: int, response: Response, token: str = Header()):
    ...
    if tokenVerification["valid"]:
        ...
        if requestedUser == requestingUser:
            try:
                leagues = Team.getTeamsOfUser(
                    userId=tokenVerification["payload"]["user-id"])['teams']
                return {
                    "success": True,
                    "leagues": leagues
                }
