...
path = 'F:\\Assignments\\Year_3\\second-semester\\Mobile Dev\\back-end\\main.db'
...
class User:
    ...
    def createUser(email: str, userName: str, password: str, age: int = 18):
        query = User.model.insert().values(
            email=email, password=password, age=age, userName=userName)
        try:
    ...
    def changePassword(id: int, password: str):
        query = update(User.model).where(
            User.model.c.id == id).values(password=password)
        try:
            connection.execute(query)
            connection.commit()
            return True
    ...
    def changeUserName(userName: str, id: int):
        query = update(User.model).where(
            User.model.c.id == id).values(userName=userName)
        try:
            connection.execute(query)
            connection.commit()
            return True
...
class Match:
    ...
    keys = ["id", "teamIdOne", "teamIdTwo", "tournamentId",
            "date", "finished", "currentState", "startTime"]
    ...
    def matchesForDate(date: int):
        ...
        try:
            ...
            for match in result:
                ...
                matchInfo = {key: value for key,
                             value in zip(Match.keys, match)}
                ...
                matchInfo['homeTeam'] = Club.getClubName(
                    matchInfo["teamIdOne"])
                matchInfo['awayTeam'] = Club.getClubName(
                    matchInfo["teamIdTwo"])
    ...
    def matchStats(matchId: int):
        ...
        try:
            ...
            matchInfo = {key: value for key,
                         value in zip(Match.keys, requestedMatch)}

            statKeys = ["score", "possession",
                        "totalShots", "shotsOnTarget", "saves"]
...
class Player:
    ...
    def filterPlayers(teamId: int = None, maxPrice: float = float("inf"), minPrice: float = 0, offset: int = 0, limit: int = 50):
        if all(x is None for x in (teamId, maxPrice, minPrice)):
            ...
            query = Player.model.select().where(Player.model.c.price <= maxPrice,
                                                Player.model.c.price >= minPrice).offset(offset=offset).limit(limit=limit)
        elif teamId != None:
            query = Player.model.select().where(Player.model.c.price <= maxPrice, Player.model.c.price >=
                                                minPrice, Player.model.c.clubId == teamId).offset(offset=offset).limit(limit=limit)
    ...
    def addPlayer(id: int, name: str, teamId: int, price: int, position: str, injured: int = 0):
        query = Player.model.insert().values(id=id, name=name, clubId=teamId,
                                             price=price, position=position, injured=injured)
        try:
            connection.execute(query)
            connection.commit()
            return True
...
class PlayerStats:
    ...
    def getAllStats(playerId: int):
        query = PlayerStats.model.select().where(PlayerStats.model.c.player_id ==
                                                 playerId).order_by(PlayerStats.model.c.date.desc())
        try:
            result = connection.execute(query).fetchall()
            playerStat = []
            for match in result:
...
class Team:
    model = Table(
        'Team', meta,
        Column('id', Integer,  ForeignKey("id", ondelete="CASCADE"),
               primary_key=True, autoincrement=True),
        Column('user_id', Integer),
        Column('league_id', Integer, ForeignKey(
    ...
    def getLeagueTeams(leagueId: int):
        query = Team.model.select().where(Team.model.c.league_id ==
                                          leagueId).order_by(desc(Team.model.c.total_points))
        try:
    ...
    def checkUser(leagueId: int, userId: int):
        query = Team.model.select().where(Team.model.c.league_id ==
                                          leagueId, Team.model.c.user_id == userId)
        try:
    ...
    def deleteTeam(teamId: int):
        ...
        try:
            ...
            LeagueAdmin.removeAdmin(
                leagueId=requestedTeam["league_id"], userId=requestedTeam["user_id"])
        ...
        try:
            ...
            for team in teams:
                ...
                try:
                    ...
                    checkAdmin = LeagueAdmin.checkAdmin(
                        leagueId=temp["leagueId"], userId=userId)
...
class TeamPlayers:
    model = Table(
        ...
        Column('team_id', Integer, ForeignKey(
            "Team_Players.team_id", ondelete="CASCADE")),
    ...
    def addPlayerToTeam(teamId: int, playerId: int, captain: int = 0, reserve: int = 0):
        query = TeamPlayers.model.insert().values(
            team_id=teamId, player_id=playerId, captain=captain, reserve=reserve)
        try:
            connection.execute(query)
            connection.commit()
            return True
...
class League:
    model = Table(
        'League', meta,
        Column('id', Integer, ForeignKey("league.id", ondelete="CASCADE"),
               primary_key=True, autoincrement=True),
    ...
    keys = ["id", "tournamentId", "name", "entryFee",
            "private", "entry_code", "date", "prize_pool"]

    @staticmethod
    def createLeague(owner: int, tournament_id: int, name: str, date: str, entry_code: str = '', private: int = 0, entry_fee: float = 0.0):
        query = League.model.insert().values(owner=owner, tournament_id=tournament_id,
                                             name=name, entry_fee=entry_fee, private=private, entry_code=entry_code, date=date)
        try:
            result = connection.execute(query)
    ...
    def updateName(leagueId: int, newName: str):
        query = update(League.model).where(
            League.model.c.id == leagueId).values(name=newName)
        try:
            connection.execute(query)
            connection.commit()
            return True
...
class LeagueAdmin:
    ...
    def getAdmins(leagueId: int):
        query = LeagueAdmin.model.select().where(
            LeagueAdmin.model.c.league_id == leagueId)
        try:
    ...
    def removeAdmin(leagueId: int, userId: int):
        query = LeagueAdmin.model.delete().where(LeagueAdmin.model.c.league_id ==
                                                 leagueId, LeagueAdmin.model.c.user_id == userId)
        try:
            connection.execute(query)
            connection.commit()
            return True
